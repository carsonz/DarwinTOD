...
from src.agents.e2e_agent import E2EAgent
...
class DialogueManager1:
    ...
    def __init__(self, config: Dict[str, Any], llm_client: LLMClient, output_dir: str):
        ...
        self.e2e_agent = E2EAgent(agent_config.get("e2e", {}), llm_client, self.ssm, self.esb_evolver, self.dataset_type)
    ...
    def start_new_dialogue(self, dialogue_id: str, domains: List[str] ,goal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 为每个agent加载或生成策略
        # for agent_type in ['e2e', 'user_sim']:
    ...
    def process_turn(self, dialogue_id: str) -> Dict[str, Any]:
        ...
        # 1. E2E处理 - 更新信念状态       
        e2e_result = self.e2e_agent.process(self.domains, user_utterance, dialogue_history, pre_belief_state, self.agent_strategies['e2e'])
        belief_state = e2e_result.get("belief_state", {})
        e2e_reason = e2e_result.get("reason", "")
        e2e_criticism = e2e_result.get("criticism", "")
        
        # 收集E2E的批判内容
        if e2e_criticism:
            need_evovle.append({
                "agent_type": "user_sim",
                "reason": e2e_criticism
            })

        # 确保system_action是字符串类型
        system_action_raw = e2e_result.get("system_action", "")
        ...
        db_query = ""
        db_results = []  # 初始化为空列表
        if e2e_result.get("db_query_needed", False):
            db_query = e2e_result.get("query", "")
            db_results = e2e_result.get("db_results", [])        
        
        system_utterance = e2e_result.get("system_utterance", "")
